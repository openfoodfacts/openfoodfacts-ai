    ...
    def start(self):
        ...
        for feature in self.features:
            if feature not in self.benchmark_data.column_names:
                raise ValueError(
                    f"The feature {feature} is not in the dataset features: {self.benchmark_data.column_names}"
                )
    ...
    def generate_synthetic_corrections(self):
        ...
        def process_fn(element: Dict) -> Dict:
            ...
            if not element["reference"]:
                element["reference"] = spellcheck.correct(
                    element["original"]
                )  # TODO: OpenAI processing (tqdm) not showing
            return element
