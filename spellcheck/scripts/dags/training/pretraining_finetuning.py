...
class TrainingPipeline(metaflow.FlowSpec):
    ...
    def pretrain(self):
        ...
        hyperparameters = OmegaConf.to_container(
            self.pretraining_conf.hyperparameters, resolve=True
        )  # Transform DictConfig to Dict
        ...
        estimator = HuggingFace(
            role=os.getenv(
                "SAGEMAKER_ROLE"
            ),  # Iam role used in training job to access AWS ressources, e.g. S3
            ...
            environment={  # environment variables used during training
                ...
                "HF_TOKEN": os.getenv(
                    "HF_TOKEN"
                ),  # required by some models, such as llama-3 or Mistral
        ...
        estimator.fit(
            wait=True
        )  # Wait for the pipeline. No need for inputs since data doesn't come from S3.
    ...
    def train(self):
        ...
        hyperparameters = OmegaConf.to_container(
            self.training_conf.hyperparameters, resolve=True
        )  # Transform DictConfig to Dict
        ...
        # Run training job
        estimator.fit(
            wait=True, inputs={"model": self.pretrained_model_artifact_uri}
        )  # Add previous model

        # Log Sagemaker training information into metaflow after training job
        self.sagemaker_training_job_id = estimator.latest_training_job.job_name
        self.model_artifact_uri = (
            self.training_conf.estimator.output_path + self.sagemaker_training_job_id
        )
    ...
    def human_evaluation(self):
        ...
        if self.do_human_eval:
            ...
            self.argilla_dataset_name = (
                self.training_conf.estimator.base_job_name
                + "-exp-key-"
                + self.experiment_key
            )
