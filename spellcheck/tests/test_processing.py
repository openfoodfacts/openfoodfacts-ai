...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ("escargot 14'%, olives 28 %", "escargot 14%, olives 28%"),
            "escargot 14%, olives 28 %",
        ),
        (
            ("farine 14&'!%, ravioli 47%", "farine 14 %, ravioli 47 %"),
            "farine 14 %, ravioli 47%",
        ),
        (
            ("farine 14 %, ravioli 47%", "farine 14'%, raviolo 47 %"),
...
@pytest.mark.parametrize(
    "inputs, expected",
    [
        (("oeuf, bœuf", "œuf, boeuf"), "oeuf, bœuf"),
        (("œuf, boeuf", "oeuf, bœuf"), "œuf, boeuf"),
        (("bœuf, œuf, ...", "boeuf, œuf, ..."), "bœuf, œuf, ..."),
...
def test_align_oe(inputs, expected):
    reference, text = inputs
    aligned_text = DataProcessor._align_oe(reference=reference, text=text)
    assert aligned_text == expected
