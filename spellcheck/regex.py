...
PERCENTAGE_REGEX = r'([0-9]{0,2})([,|.|;| ]{0,2})([0-9]{0,2})[ ]?([%|\?|?L|/|\\])'


def format_percentages(txt, keep_length=False):
    formatted_txt_list = []
    last_index = 0
    for match in re.finditer(PERCENTAGE_REGEX, txt):
        ...
        if keep_length:
            formatted_txt_list.append(' '*(match.end()-match.start()-len(formatted_match)))
