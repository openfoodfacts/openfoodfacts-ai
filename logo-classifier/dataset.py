...
class LogoDataset(t_data.Dataset):
    ...
    def __init__(
        self, hdf5_file: Path, transforms: List[str], prohibited_classes: List[int]
    ):
...
def get_weights(
    datasets_list: List[LogoDataset], loader_batch_size: int, num_threads: int
):
    res_list = []
    for dataset in datasets_list:
        amount_dict = {}
        data_gen = t_data.DataLoader(
            dataset=dataset, batch_size=loader_batch_size, num_workers=num_threads
        )
...
def get_dataloader(
    ...
    train_dataset, valid_dataset, test_dataset = get_datasets(
        train_path, val_path, test_path, transforms, prohibited_classes
    )

    # define samplers for train, val and test
    if debugging:
        test_weights = get_weights([test_dataset], loader_batch_size, num_threads)
        test_sampler = t_data.WeightedRandomSampler(
            weights=test_weights[0], num_samples=len(test_dataset)
        )
        test_loader = t_data.DataLoader(
        ...
        test_loader = t_data.DataLoader(
            dataset=test_dataset, batch_size=loader_batch_size, num_workers=num_threads
        )
        return test_loader, test_loader, test_loader

    train_weights = get_weights([train_dataset], loader_batch_size, num_threads)[0]

    train_sampler = t_data.WeightedRandomSampler(
        weights=train_weights, num_samples=len(train_dataset)
    )

    # create dataloader for train, val and test
    train_loader = t_data.DataLoader(
        dataset=train_dataset,
        batch_size=loader_batch_size,
        num_workers=num_threads,
        sampler=train_sampler,
    )
    valid_loader = t_data.DataLoader(
        dataset=valid_dataset, batch_size=loader_batch_size, num_workers=num_threads
    )
    test_loader = t_data.DataLoader(
        dataset=test_dataset, batch_size=loader_batch_size, num_workers=num_threads
    )
...
if __name__ == "__main__":
    ...
    train_loader, val_loader, test_loader = get_dataloader(
        train_path, val_path, test_path, ["identity_transform"], 1, 15
    )
