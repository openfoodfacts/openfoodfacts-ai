...
class ImagePreloader(t_data.Dataset):
    ...
    def __init__(self, classes: list, json_dataset: str, missed_logos: dict):
        ...
        for dict in tqdm.tqdm(data_gen):
            id = str(dict["logo_id"])
            if id in set_ids and (
                classes == [] or missed_logos[id]["prediction"] in classes
            ):
                self.ids.append(id)
...
def get_classes(input_json_file):
    ...
    for logos_id in missed_logos.keys():
        ...
        truth, prediction = get_str_labels(
            [truth, prediction], classes_ids, classes_str
        )
...
def generate_images(source_img: str, bounding_box: list):
    ...
    try:
        ...
        print(
            f"This one has a shape problem : source_img and shape = {np.shape(image)}"
        )
        return None
...
def save_image(id, predicted, true, image, score):
    ...
    if not os.path.isfile(
        base_dict + class_predicted_dict + true_class_dict + image_title
    ):
        try:
...
if __name__ == "__main__":
    ...
    *logos_infos_file should contain the path of a jsonl file where each line is a dictionnary with
    ...
    dataloader = t_data.DataLoader(
        dataset, batch_size=32, num_workers=4, collate_fn=custom_collate_fn
    )
