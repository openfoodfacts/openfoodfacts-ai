...
def train(
    ...
    y_test = []
    y_pred = []
    classes_str, classes_ids = get_labels(
        settings.labels_path, prohibited_classes=prohibited_classes
    )
    ...
    for epoch in range(epochs):
        ...
        for embeddings, labels, ids in tqdm.tqdm(train_loader):
            ...
            ground_truth = torch.tensor(
                [torch.where(classe == 1)[0] for classe in labels]
            )
        ...
        with torch.no_grad():
            ...
            for embeddings, labels, ids in tqdm.tqdm(val_loader):
                ...
                for indice in torch.where(
                    (predicted == ground_truth.to(device)) == False
                )[0].tolist():
                    missed_no_logos[str(ids[indice].item())] = [
                        ground_truth[indice].item(),
                        predicted[indice].item(),
                        scores[indice][predicted[indice]].item(),
                    ]

                if (
                    not valid_no_class
                ):  # To compute the metrics, we take out all no_class logos
        ...
        current_macro_f1 = compute_metrics(
            y_test,
            y_pred,
            classes_ids,
            classes_str,
        ...
        save_best_model(
            current_macro_f1, missed_no_logos, epoch, model, device, files_dir
        )
...
def compute_metrics(
    ...
    report = sklearn.metrics.classification_report(
        y_test, y_pred, labels=classes_ids, target_names=classes_str, zero_division=0
    )
    f1_micro = sklearn.metrics.f1_score(
        y_true=y_test, y_pred=y_pred, labels=classes_ids, average="micro"
    )
    ...
    f1_classes = sklearn.metrics.f1_score(
        y_true=y_test, y_pred=y_pred, labels=classes_ids, average=None, zero_division=0
    )
    ...
    ax = plt.gca()
    ax.set_xticks(np.arange(-0.5, len(classes_str) - 1, 1))
    ax.set_yticks(np.arange(-0.5, len(classes_str) - 1, 1))
    ax.set_xticklabels(
        classes_str,
        size="smaller",
        rotation="vertical",
...
class SaveBestModel:
    ...
    def __init__(self, best_macro_f1=-float("inf")):
