...
def generate_embeddings_iter(
    ...
    with h5py.File(str(file_path), "r") as f:
        ...
        for slicing in chunked(range(len(image_dset)), batch_size):
            slicing = np.array(slicing)  # slicing was a list, we make an array out of it
            external_ids = external_id_dset[slicing]
            mask = external_ids == 0

            if np.all(mask):  # if we only have zeros as external ideas, it's an empty batch and it means we won't have any other logos so we stop the loop
                break

            mask = (~mask) & (confidence_dset[slicing] >= min_confidence)  # we add to the mask the confidence parameter to avoid embedding 
            ...
            with torch.no_grad():
                array_to_PIL = lambda x: PIL.Image.fromarray(x,mode="RGB")  # convert the np.array to PIL in order to use the CLIProcesspr
                torch_images = torch.tensor(images, dtype=torch.float32, device=device).cpu().numpy()
                images = processor(images=[array_to_PIL(torch_images[i]) for i in range(batch_size)], return_tensors="pt")["pixel_values"]
                embeddings = model(**{"pixel_values": images.to(device)}).pooler_output.cpu().numpy()
...
if __name__ == "__main__":
    ...
    assert (
        args.model_type.startswith("clip-vit-base-patch") and args.model_type[-1].isdigit()
    )
