...
def create_index(
    ...
    count = 0
    if os.path.exists("percentage_study.txt"):
        os.remove("percentage_study.txt")
    ...
    if exact:
        ...
        while percent != "1":
            ...
            with open("percentage_study.txt", "a") as f:
                f.write(
                    f"Percentage of Flat indexed {percent} at time {count*20} sec \n"
                )
        ...
        while percent != "1":
            ...
            with open("percentage_study.txt", "a") as f:
                f.write(
                    f"Percentage of HNSW indexed {percent} at time {count*240} sec \n"
                )
    ...
    for batch in tqdm.tqdm(data_gen):
        ...
        for i in range(len(embeddings_batch)):
            ...
            client.hset(
                i + offset,
                mapping={
                    embedding_field_name: embeddings_batch[i]
                    .astype("float32")
                    .tobytes(),
...
def search_index(
    ...
    for batch in tqdm.tqdm(data_gen):
        ...
        for i in range(len(embeddings_batch)):
            ...
            q = (
                Query(f"*=>[KNN $k @{embedding_field_name} $vec_param AS dist]")
                .paging(0, 101)
                .sort_by(f"dist")
            )

            start_time = time.monotonic()
            res = client.ft(idx_name).search(
                q,
                query_params={
                    "k": int(K.max() + 1),
            ...
            nearest_neighbours[query_id]["ids"] = np.array(
                [int(doc.external_id) for doc in res.docs]
            )
            nearest_neighbours[query_id]["distances"] = np.array(
                [float(doc.dist) for doc in res.docs]
            )
...
def evaluate(
    ...
    if pathlib.Path(KNN_file).is_file() and pathlib.Path(perf_file).is_file():
        ...
        performances["ground_truth"]["creation_time"] = create_index(
            client, embedding_field_name, external_id_field_name, d, exact=True
        )

        if client.dbsize() != index_size and client.dbsize() < 4317343:
            performances["ground_truth"]["building_time"] = build_index(
                client,
                embeddings_path,
                embedding_field_name,
        ...
        performances["ground_truth"]["ram_used"] = (
            ram_available_before - ram_available_after
        ) / (1024.0**3)

        print("Search in FLAT index to compute ground truth")

        ground_truth, performances["ground_truth"]["search_time"] = search_index(
            client, True, embeddings_path, queries, batch_size, embedding_field_name, K
        )
    ...
    for m in M_list:
        ...
        for ef_construction in EF_CONSTRUCTION_list:
            ...
            for ef_runtime in EF_RUNTIME_list:
                ...
                if (
                    pathlib.Path(perf_file).is_file()
                    and pathlib.Path(KNN_file).is_file()
                ):
                    ...
                    if client.dbsize() != index_size and client.dbsize() < 4317343:
                        hnsw_perf["bulding_time"] = build_index(
                            client,
                            embeddings_path,
                            embedding_field_name,
                            external_id_field_name,
                    ...
                    hnsw_nearest_neighbours, hnsw_perf["search_time"] = search_index(
                        client,
                        False,
                        embeddings_path,
                        queries,
