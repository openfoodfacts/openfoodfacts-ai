...
def compute_recall(
    ...
    for id in ground_truth.keys():
        ...
        for k in K:
            k = int(k)
            positive_neighbours = np.isin(
                approx_res[id]["ids"][1 : k + 1], ground_truth[id]["ids"][1 : k + 1]
            )
            ...
            found_neighbours_among_the_expected_ones = np.isin(
                ground_truth[id]["ids"][1 : k + 1], approx_res[id]["ids"][1 : k + 1]
            )
            fn = k - (np.sum(found_neighbours_among_the_expected_ones.astype(int)))
            fn_micro[k] = fn_micro[k] + fn

            performances["macro_recall"][k] = performances["macro_recall"][k] + tp / (
                tp + fn
            )
            performances["macro_precision"][k] = performances["macro_precision"][
                k
            ] + tp / (tp + fp)
...
if __name__ == "__main__":
    ...
    save_dir = (
        "computation_512_recall_index_" + str(index_size) + "_queries_" + str(queries)
    )

    ground_truth = load_data(
        "faiss_saves_index_4371343_queries_1000/512_exact_KNN.json"
    )
    # approx_res = load_data("PCA_300_saves_index_4371343_queries_1000/hnsw_ANN_"+str(M)+"_"+str(ef_construction)+"_"+str(ef_search)+".json")
    approx_res = load_data(
        "elastic_index_4371343_queries_1000/512_ef_hnsw_ANN_ncandidates_110_m_16_efconstruction_100.json"
    )

    complete_perf = compute_recall(
        ground_truth,
        approx_res,
        index_size,
        queries,
