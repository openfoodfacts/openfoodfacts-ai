...
def create_index(
    d: int, exact: bool, m: int = 32, efSearch: int = 40, efConstruction: int = 40
):
    """
...
def build_index(
    ...
    for batch in tqdm.tqdm(data_gen):
        ...
        index.add_with_ids(
            embedding_batch.astype("float32"), external_id_batch.astype("int64")
        )
...
def search_index(
    ...
    for batch in tqdm.tqdm(data_gen):
        ...
        for i in range(len(embeddings_batch)):
            ...
            res = index.search(
                np.array([embeddings_batch[i]]).astype("float32"), int(K.max()) + 1
            )
...
def main(
    ...
    if pathlib.Path(KNN_file).is_file() and pathlib.Path(perf_file).is_file():
        ...
        performances["ground_truth"]["building_time"] = build_index(
            index, embeddings_path, batch_size, index_size
        )
        ...
        ground_truth, performances["ground_truth"]["search_time"] = search_index(
            index, embeddings_path, queries, batch_size, K
        )
        ...
        for ef_construction in EF_CONSTRUCTION_list:
            ...
            for ef_search in EF_SEARCH_list:
                ...
                if (
                    ...
                    hnsw_perf["building_time"] = build_index(
                        index, embeddings_path, batch_size, index_size
                    )
