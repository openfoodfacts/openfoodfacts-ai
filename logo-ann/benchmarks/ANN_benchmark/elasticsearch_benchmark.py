...
def create_index(
    es: Elasticsearch, index_name: str, d: int, ef_construction: int, m: int
):
    """
    ...
    for batch in tqdm.tqdm(data_gen):
        ...
        for i in range(len(embeddings_batch)):
            ...
            nearest_neighbours[query_id]["ids"] = np.array(
                [hits[i]["_source"]["external_id"] for i in range(len(hits))]
            )
            nearest_neighbours[query_id]["score"] = np.array(
                [hits[i]["_score"] for i in range(len(hits))]
            )
...
if __name__ == "__main__":
    ...
    es = Elasticsearch(
        hosts="http://elastic:elastic_pswd@localhost:9200", verify_certs=False
    )
    es.info().body
