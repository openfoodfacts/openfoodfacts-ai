...
class Evaluator:
    ...
    def classification_report(
        ...
        df_report.rename(
            columns={colname: name + "_" + colname for colname in df_report.columns},
            inplace=True,
        )
    ...
    def global_metrics(self, average="weighted", name="Model", zero_div=0):
    ...
    def plot_confidence(
        ...
        g.map(sns.kdeplot, "pred_confidence", fill=True, common_norm=True, alpha=0.4)
        g.add_legend()
    ...
    def plot_confusion_matrix(
        ...
        plt.figure(figsize=figsize)
        plot = sns.heatmap(
            cm, xticklabels=labels, yticklabels=labels, annot=annot, cmap=cmap, fmt="g"
        )
