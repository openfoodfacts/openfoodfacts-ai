...
class XGFood:
    ...
    def process_X(
        ...
        if not self._feat_cols_filled:
            ...
            self.raw_cols_for_ings = [
                col for col in self.features_cols if "ing_" in col
            ]
            self.cols_for_ings = [
                re.sub("ing_", "", col) for col in self.raw_cols_for_ings
            ]
            self.cols_for_text = [
                col for col in self.features_cols if "ing_" not in col
            ]
        ...
        if verbose:
            print(f"Processing done - Total running time : {round(time_proc_end,2)}mn")
        return self.X

    def process_ingredients(self):
        # Convert object to original structured list of dictionnaries
        self.X_raw[self.ingredients_column] = self.X_raw[self.ingredients_column].apply(
            literal_eval
        )
        # Loop in the list of dicts
        for ingre_list, index in zip(
            self.X_raw[self.ingredients_column], self.X_raw.index
        ):
            # Loop in the dicts
            for ingre_dict in ingre_list:
                ...
                if val_clean in self.cols_for_ings:
                    ...
                    try:
                        self.X_raw.loc[index, "ing_" + val_clean] = ingre_dict[
                            "percent_estimate"
                        ]
    ...
    def filter_preds(self, y_probas, tresholds):
        ...
        for p in y_probas:
            ...
            if result.size > 0:
                preds.append(int(result[0]))
            else:
                preds.append(y_unknown_code)
    ...
    def decode_labels(self, preds, label_dict):
        preds_decoded = []
        for pred in preds:
            preds_decoded.append(label_dict[str(pred)])
        ...
        if pred_format == "np":
            if get_confidence:
                ...
                predictions = np.append(
                    y_preds_G1.reshape(-1, 1), y_preds_G2.reshape(-1, 1), axis=1
                )
