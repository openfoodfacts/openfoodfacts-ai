...
def get_taxonomy_info(category=str, info_type="tax_list"):
    ...
    if not isinstance(cat_taxonomy, nonetype):
        cat_childs_list = [
            re.sub("<TaxonomyNode ", "", str(child))[:-1]
            for child in cat_taxonomy.children
        ]
        cat_parents_list = [
            re.sub("<TaxonomyNode ", "", str(parent))[:-1]
            for parent in cat_taxonomy.parents
        ]
        cat_tax_list = cat_childs_list + cat_parents_list

        # Set up output depending on information passed in arguments
        if info_type == "childs":
            return cat_childs_list
        elif info_type == "parents":
            return cat_parents_list
        elif info_type == "full_tax_list":
            return cat_tax_list
...
def find_pnns(
    row=None, df_taxonomy=None, pnns_n=1, nb_groups=4, search_duplicates=True
):
    """
    ...
    tax_list = [
        get_taxonomy_info(tags_list[i], info_type="parents")
    ...
    for suggestion, pnns in zip(df_taxonomy.taxonomy_suggestion, df_taxonomy.pnns):
        if suggestion in tax_list:
            pnns_candidates.append(pnns)
    # find candidates in others possibilities
    for possibilities, pnns in zip(
        df_taxonomy.all_taxonomy_possibilities, df_taxonomy.pnns
    ):
...
def find_pnns_groups_1(df):
    ...
    for val in vals_to_find:
        group_2_vals = list(
            data["pnns_groups_2"].loc[data["pnns_groups_1"] == val].unique()
        )
        data["pnns_groups_1"].loc[
            (data["pnns_groups_1"] == "unknown")
            & (data["pnns_groups_2"].isin(group_2_vals))
        ] = val
    return data
...
def get_ingredients_columns(df, ingredients_list):
    ...
    for i in ingredients_list:
        data[i] = 0  # create 0 columns, 1 per ingredient
    for ingre_list, index in zip(data.ingredients, data.index):  # loop over df rows
        for ingre_dict in ingre_list:  # loop continue in the dicts in each row
            ...
            if val_clean in ingredients_list:  # check if val is in columns added
                try:
                    data.loc[index, val_clean] = ingre_dict[
                        "percent_estimate"
                    ]  # if yes, replace by estimate
                except:
                    pnns_val = data.loc[index, "pnns_groups_1"]
                    val_median = (
                        data[val_clean].loc[data["pnns_groups_1"] == pnns_val].median()
                    )
...
def depreciated_find_pnns(row, df_taxonomy):
    ...
    if not isinstance(tax_list, float):
        ...
        if isinstance(output, float):
            for possibilities, pnns in zip(
                df_taxonomy.all_taxonomy_possibilities, df_tax.pnns
            ):
                for possibility in possibilities.keys():
                    if possibility in tax_list:
                        output = pnns
