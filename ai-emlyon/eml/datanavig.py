...
def remove_cols_dict(cols_dict, cols_to_drop):
    if isinstance(cols_to_drop, dict):
        cols_to_drop = [
            col for col in cols_to_drop.keys()
        ]  # extract columns names in a list
    for group in cols_dict.keys():  # loop over the cols dict
        cols_dict[group] = [
            col for col in cols_dict[group] if col not in cols_to_drop
        ]  # keep only cols names not in cols_to_drop
...
off_columns_dict = {
    ...
    "dummy": ["pnns_groups_1", "pnns_groups_2", "nutriscore_grade"],
    "tags": [
        "categories_tags",
        "manufacturing_places_tags",
        "labels_tags",
